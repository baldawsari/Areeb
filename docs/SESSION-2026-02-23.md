...
### Priority 1 — Dashboard WebSocket Handshake

Dashboard at `localhost:3000` fails to connect to gateway every ~30s (`invalid request frame`). Likely auth token mismatch or connect frame format issue in `dashboard/src/lib/gateway.js`.

...
| 2 | Register 6 Agents | DONE — all in agents.list |
...
| 5 | Re-enable QMD Memory Backend | DEFERRED — markdown backend works for now |
| 6 | Enable Slack and Voice | DEFERRED — needs Slack app setup |
| 7 | Security: Rotate Exposed Keys | **ACTION NEEDED** — .env not tracked, but keys may be in git history |

## Next Steps

1. **Stop local gateway** (port 18789 is occupied by the local instance)
2. **Start Docker**: `docker compose up -d`
3. **Verify gateway starts**: `docker logs -f agent-team-gateway`
4. **Test Telegram**: Send a message to @AreebAgentsBot
5. **Test dashboard**: Open `http://localhost:3000` — should show "Live" indicator when connected
6. **Rotate API keys** if they were ever in git history
7. **Build custom Dockerfile** to avoid reinstalling git+openclaw on every container restart
8. **Add per-agent routing** — route to specific agents based on Telegram commands or keywords

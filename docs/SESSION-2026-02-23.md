...
### Priority 2 — Bridge Agent Tasks to React Dashboard

Agents (scrum-master) create tasks in markdown files (`BOARD.md`) inside the container, but the React dashboard stores tasks in browser localStorage — they're disconnected. Need to unify so agent-created tasks appear in the dashboard automatically.
- Option A: Dashboard reads/parses `BOARD.md` from agent workspace via gateway (needs operator.read scope or custom endpoint)
- Option B: Agents push tasks via a shared API/event that the dashboard listens to
- Option C: Gateway exposes agent workspace files in snapshot or as an event when modified
- Related: scrum-master currently has 1 task ("Brainstorm new project: Areeb") in Sprint 1 that doesn't show in dashboard

### Priority 3 — Device Identity for Full Dashboard Access

Token auth only grants `operator.admin`. For full dashboard with sessions.list, chat.send, etc.:
- Generate Ed25519 keypair in browser (Web Crypto API or pre-generated)
- Sign challenge nonce with v2 payload format
- Store deviceToken from hello-ok for reconnection
- Alternative: configure gateway to grant broader scopes with token auth
...
### Priority 7 — EBUSY Config Warning
